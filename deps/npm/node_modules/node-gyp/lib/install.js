...
function install (fs, gyp, argv, callback) {
  ...
  function go () {
    ...
    mkdir(devDir, function (err, created) {
      ...
      if (process.platform === 'os390' && tarPath) {
        var sp = spawn('pax', ['-s#node-v[0-9]*.[0-9]*.[0-9]*/##p', '-rzf', tarPath], {cwd: devDir})
        sp.on('error', cb)
        sp.on('close', afterTarball)
        return
      }
      else if(process.platform === 'os390') {
        var sp = spawn('cp', [ '-r', path.dirname(process.execPath) + '/../include', '.' ], {cwd: devDir})
        sp.on('error', cb)
        sp.on('close', afterTarball)
        return
      }
      ...
      function afterTarball () {
        ...
        if (!tarPath && process.platform != 'os390') {
