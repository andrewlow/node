...
] : []).concat(process.platform === 'os390' ? [
  ['__UNTAGGED_READ_MODE', { 
    helpText: 'Set to "AUTO" (default) for handling of reading untagged ' +
              'files, up to 4k of data will be read and checked, ' +
              'if it is found to be in ccsid 1047, convert data ' +
              'from ccsid 1047 to ccsid 819. Set to "WARN" for ' +
              'same behavior as DEFAULT but issue a warning if ' +
              'conversion occurs. Set to "V6" for same behavior ' +
              'as node v6/v8, i.e. always convert data from ccsid ' +
              '1047 to ccsid 819.  Set to "STRICT" for no ' +
              'explicit conversion of data' }],
  ['DISABLE_NODEJS_SMF89_REGISTRATION', { 
    helpText: 'If set, disables the creation of SMF 89 records with IFAUSAGE ' +
              'for tailored fit pricing' }],
  ['NODEJS_SMF89_REGISTRATION_VERBOSE', { 
    helpText: 'If set, displays verbose SMF 89 record data' }],
  ['NODEJS_SMF89_MAJOR_VERSION', { 
    helpText: 'Override major version for SMF 89 record registration' }],
  ['NODEJS_SMF89_PRODUCT_OWNER', { 
    helpText: 'Override product owner for SMF 89 record registration' }],
  ['NODEJS_SMF89_FEATURE_NAME', { 
    helpText: 'Override feature name for SMF 89 record registration' }],
  ['NODEJS_SMF89_PRODUCT_NAME', { 
    helpText: 'Override product name for SMF 89 record registration' }],
  ['NODEJS_SMF89_PID', { 
    helpText: 'Override pid name for SMF 89 record registration' }],
] : []));
