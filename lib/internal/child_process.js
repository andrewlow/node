...
ChildProcess.prototype.spawn = function(options) {
  ...
  // Prepend z/OS envars so that they can be overriden by env param
  if (process.platform === 'os390') {
    let zosEnvPairs = [];
    if (process.env._BPXK_AUTOCVT)
      zosEnvPairs.push('_BPXK_AUTOCVT=' + process.env._BPXK_AUTOCVT);
    if (process.env._CEE_RUNOPTS)
      zosEnvPairs.push('_CEE_RUNOPTS=' + process.env._CEE_RUNOPTS);
    if(process.env._TAG_REDIR_ERR)
      zosEnvPairs.push('_TAG_REDIR_ERR=' + process.env._TAG_REDIR_ERR);
    if (process.env._TAG_REDIR_IN)
      zosEnvPairs.push('_TAG_REDIR_IN=' + process.env._TAG_REDIR_IN);
    if (process.env._TAG_REDIR_OUT)
      zosEnvPairs.push('_TAG_REDIR_OUT=' + process.env._TAG_REDIR_OUT);
    if (process.env.STEPLIB)
      zosEnvPairs.push('STEPLIB=' + process.env.STEPLIB);
    if (process.env.LIBPATH)
      zosEnvPairs.push('LIBPATH=' + process.env.LIBPATH);

    options.envPairs = zosEnvPairs.concat(options.envPairs || []);
  }
